...
scene.set_background_color((0.1, 0.2, 0.4))
scene.set_directional_light((1, 0.9, -1), 0.2, (1, 1, 0.8))
...
def create_tree(pos, height, radius, color):
    create_block(pos, ivec3(3, height, 3), vec3(0.7), vec3(0.3))
    ...
    for i, j in ti.ndrange((-radius, radius), (-radius, radius)):
        ...
        if ti.random() < prob * prob:
            scene.set_voxel(pos + ivec3(i, 1, j), 1, color)

@ti.kernel
def initialize_voxels():
    for i in range(4):
        create_block(ivec3(-60, -(i + 1) ** 2 - 40, -60), ivec3(120, 2 * i + 1, 120), vec3(0.5 - i * 0.1) * vec3(1.0, 0.8, 0.6), vec3(0.05 * (3 - i)))

    create_block(ivec3(-60, -40, -60), ivec3(120, 1, 120), vec3(0.3, 0.2, 0.1), vec3(0.01))

    create_tree(ivec3(-30, -40, -30), 30, 20, vec3(0.9, 0.6, 0.1))
    create_tree(ivec3(-20, -40, 25), 65, 35, vec3(1.0, 0.4, 0.1))
